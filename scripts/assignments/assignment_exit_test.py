...
class UserBehavior(SequentialTaskSet):
    ...
    def on_start(self):
        if len(credentials.UserNames) > 0:
            self.username, self.password = credentials.UserNames.pop()
            print(self.username, self.password)
            credentials.UserNames.insert(0, (self.username, self.password))
        else:
    ...
    def select_autoquote(self):
        with self.client.get("/InsuranceWebExtJS/quote_auto.jsf_wrong_url", name="select autoquote",
